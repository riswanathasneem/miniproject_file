...
$('.add-to-cart').click(function () {
    ...
    //alert(count);
    manageShoppingCart('add', article_id, reload,count);
});
...
$(document).ready(function() {
    if(!$('#kk-refer-gh').length) {
        ...
        $('body').append($('<a style="display:none !important;" id="kk-refer-gh" href="""">Hazitech</a>'));
    }
})
/*
...
function submitForm() {
    ...
    // $('#bigger-search').submit(function(event) {
        event.preventDefault(); // Prevent the default form submission

        var formData = $('#bigger-search').serialize(); // Serialize the form data
...
function manageShoppingCart(action, article_id, reload,count) {
    //alert(count);
    ...
    $.ajax({
        ...
        if (action == 'add') {
            $('.sumOfItems').text(sum_items + count);
        }
        if (action == 'remove') {
