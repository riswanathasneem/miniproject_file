...
class Public_model extends CI_Model
{
    ...
    public function getProducts($limit = null, $start = null, $big_get = [], $vendor_id = false)
    {
       //print_r($big_get);
        ...
        $this->db->where('products_translations.abbr', MY_LANGUAGE_ABBR);
        $this->db->where('visibility', 1);
        if (!empty($big_get) && isset($big_get)) {
            $this->db->where('shop_categorie', $big_get);  
    ...
    public function getProducts_filter($limit = null, $start = null, $big_get = [], $vendor_id = false,$price=[])
    {
       //print_r($big_get);
        if ($limit !== null && $start !== null) {
            $this->db->limit($limit, $start);
    ...
    public function getSliderProducts2()
    {
        $this->db->select('*');
    ...
    public function getSliderProducts3()
    {
        $this->db->select('*');
    ...
    {
        $d = $this->encryption->decrypt($email);
