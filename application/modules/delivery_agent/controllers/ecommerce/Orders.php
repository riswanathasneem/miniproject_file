...
class Orders extends ADMIN_Controller
{
    ...
    public function index($page = 0)
    {
        ...
        $data['links_pagination'] = pagination('delivery_agent/orders', $rowscount, $this->num_rows, 3);
        if (isset($_POST['paypal_sandbox'])) {
            ...
            redirect('delivery_agent/orders?settings');
        }
    ...
    public function deleteOrder($id)
    {
        $id = (int) $id;
        if($id == 0) {
            redirect('delivery_agent/orders');
        }
    ...
    private function sendVirtualProducts()
    {
        if(isset($_POST['products']) && $_POST['products'] != '') {
            ...
            foreach ($products as $product_id => $product_quantity) {
                $productInfo = modules::run('delivery_agent/ecommerce/products/getProductInfo', $product_id);
                /*
