...
class Slider extends ADMIN_Controller
{
    ...
    public function index($id = 0)
    {
        ...
        $trans_load = null;
        if ($id > 0 && $_POST == null) {
            $_POST = $this->Slider_model->getOneProduct($id);
            //$trans_load = $this->Slider_model->getTranslations($id);
        }
        if (isset($_POST['submit'])) {
            ...
            $this->Slider_model->setProduct($_POST, $id);
            $this->session->set_flashdata('result_publish', 'Slider is Added!');
            if ($id == 0) {
                $this->saveHistory('Success slider Added');
            } else {
            ...
            if (isset($_SESSION['filter']) && $id > 0) {
                ...
                redirect(base_url('admin/sliderview?' . $get));
            } else {
                redirect('admin/sliderview');
            }
        }
        $data = array();
        $head = array();
        ...
        $this->load->view('ecommerce/slider', $data);
