...
public function orders($limit, $page, $order_by)
{
    ...
    public function changeOrderStatus($id, $to_status)
    {
        ...
        $result = true;
        if ($res['delivered'] != $to_status) {
            $this->db->where('id', $id);
            $result = $this->db->update('orders', array('delivered' => $to_status, 'viewed' => '1'));
            if ($result == true) {
                $this->manageQuantitiesAndProcurement($id, $to_status, $res['delivered']);
            }
        }
        return $result;
    }
